...
{
  if (!odometry_) {
    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000 /*ms*/, "odometry is not available");
    return false;
  }
  if (!trajectory_) {
    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000 /*ms*/, "trajectory is not available");
    return false;
  }
  if (trajectory_->points.empty()) {
    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000 /*ms*/,  "trajectory points is empty");
    return false;
  }
  return true;
}
